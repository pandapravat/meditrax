...
public final class AutoCompleteTextFieldBehavior<T> extends BehaviorBase<AutoCompleteTextField<T>> {
    ...
    private ChangeListener<String> textChangeListener = new ChangeListener<String>() {
        ...
        public void changed(ObservableValue<? extends String> observable, String oldValue, String newValue) {
            ...
            if (newValue != null && oldValue.length() > 0 && newValue.startsWith(oldValue)) {
                ...
                if (refText.length() > 0) {
                	// if an instant Data provider is specified then then get the data from data provider
                	// and populate in the control items
                	InstantDataProvider<T> dataProvider = getControl().getDataProvider();
                	ObservableList<T> items = getControl().getItems();
                	if(null != dataProvider) {
                		items.clear();
                		items = dataProvider.getData(refText);
                	}
                    extractMatchingItems(items, refText);
                }
            }
    ...
    private void selectAutoCompleteItem() {
        ...
        if (item != null) {
            ...
            // After selecting the item call the handle method of after selected
            // data handler of the AutoCompleteTextBox
            if(null != getControl().getAfterSelectDataHandler()) {
            	getControl().getAfterSelectDataHandler().handle(item);
            }
        }
    }
